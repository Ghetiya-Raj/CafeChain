...
const sendEmail = async (to, subject, html) => {
  try {
      const { data, error } = await resend.emails.send({
          from: 'CafeChain Partners <team@cafechain.in>', // Using a different 'from' for partners
          to: to,
          subject: subject,
          html: html,
      });
      ...
      console.log('✅ Email sent via Resend. Email ID:', data.id);
      return true;
  } catch (error) {
...
exports.requestCafeEmailOTP = async (req, res) => {
  ...
  try {
    ...
    const emailSent = await sendEmail(email, 'Confirm Your CafeChain Partner Account', emailHtml);
    ...
    //   to: email,
    //   subject: 'Confirm Your CafeChain Partner Account',
    //   html: `
    //   <!DOCTYPE html>
    //   <html lang="en">
    //   <head>
    ...
    //                           <td align="center" style="padding: 20px 30px; border-top: 1px solid #eeeeee;">
    //                               <p style="color: #aaaaaa; font-size: 12px; margin: 0;">© 2025 CafeChain. All rights reserved.</p>
    //                               <p style="color: #aaaaaa; font-size: 12px; margin: 5px 0 0 0;">You're receiving this because you started the registration process for a cafe on our platform.</p>
    //                           </td>
    //                       </tr>
...
exports.initiateRedemption = async (req, res) => {
  ...
  try {
    ...
    //   from: process.env.EMAIL_USER,
    //   to: customer.email,
    //   subject: `That's Loyalty! Your CafeChain Reward Awaits...`,
    //   html: `
    //   <!DOCTYPE html>
    //   <html lang="en">
    //   <head>
...
exports.requestPasswordReset = async (req, res) => {
  ...
  //       from: process.env.EMAIL_USER,
  //       to: email,
  //       subject: 'Your CafeChain Password Reset Code',
  //       html: `
  //       <!DOCTYPE html>
  //       <html lang="en">
  //       <head>
  ...
  //     await transporter.sendMail(mailOptions);
      ...
      const emailHtml = `
        ...
        <head>
            ...
            <title>CafeChain Password Reset</title>
            <style>
                /* styles are the same, omitted for brevity */
            </style>
        </head>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333333;">
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
      ...
      const emailSent = await sendEmail(email, 'Your CafeChain Password Reset Code', emailHtml);
...
//   try {
//       const offer = await Offer.findOne({ _id: offerId, cafe: cafeId });

//       if (!offer) {
//           return res.status(404).json({ error: "Offer not found or you do not have permission to edit it." });
//       }

//       offer.name = name;
//       offer.pointsRequired = pointsRequired;
//       await offer.save();
