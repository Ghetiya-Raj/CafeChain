...
const sendEmail = async (to, subject, html) => {
  try {
      const { data, error } = await resend.emails.send({
          from: 'CafeChain <team@cafechain.in>', // IMPORTANT: Use a verified domain with Resend
          to: to,
          subject: subject,
          html: html,
      });
...
exports.sendOtp = async (req, res) => {
  try {
    ...
    const emailSent = await sendEmail(user.email, "Your CafeChain Password Reset Code", emailHtml);
