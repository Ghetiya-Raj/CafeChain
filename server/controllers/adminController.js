...
const sendEmail = async (to, subject, html) => {
    try {
        const { data, error } = await resend.emails.send({
            from: 'CafeChain Admin <team@cafechain.in>',
            to: to,
            subject: subject,
            html: html,
        });
...
exports.approveCafe = async (req, res) => {
  try {
      ...
      if (!emailSent) {
          console.error('Failed to send approval email to:', pendingData.email);
...
exports.rejectCafe = async (req, res) => {
  try {
      ...
      if (!emailSent) {
          console.error('Failed to send rejection email to:', pendingCafe.email);
          // Continue with the process even if email fails
      }
...
exports.testAdminEmail = async (req, res) => {
  try {
      const emailSent = await sendEmail(
          ...
          '✅ CafeChain Admin Email System Test',
          `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h1 style="color: #6F4E37;">✅ CafeChain Admin Email System Active</h1>
              <p>This is a test email sent from the <strong>Admin Controller</strong> using Resend.</p>
      ...
      if (emailSent) {
          return res.json({ 
              success: true, 
              message: 'Admin test email sent to team.cafechain@gmail.com' 
          });
      } else {
          return res.status(500).json({ success: false, message: 'Admin email test failed' });
      }
  } catch (error) {
...
exports.getContactSubmissions = async (req, res) => {
  try {
    // Fetch all submissions and sort them with the newest ones first
...
exports.deleteAllContactSubmissions = async (req, res) => {
  try {
    await ContactUs.deleteMany({}); // An empty filter deletes all documents
