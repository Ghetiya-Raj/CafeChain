...
const sendEmail = async (to, subject, html) => {
    try {
        const { data, error } = await resend.emails.send({
            from: 'CafeChain <team@cafechain.in>', // Use your domain
            to: to,
            subject: subject,
            html: html,
        });
...
exports.register = async (req, res) => {
    ...
    try {
        ...
        // Send email
        const emailSent = await sendEmail(email, 'Welcome to CafeChain! Please Verify Your Email', emailHtml);
...
exports.testEmail = async (req, res) => {
    try {
        const emailSent = await sendEmail(
            ...
            '<h1>🎉 Email Test Successful!</h1><p>Your CafeChain email service is working correctly with Resend and Amazon SES.</p>'
        );
