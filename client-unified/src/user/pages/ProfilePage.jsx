...
//                                 <Link to="/user/invoice-history" className="group flex items-center justify-between p-4 rounded-lg bg-white hover:bg-[#4a3a2f] hover:text-white transition-colors border border-stone-200">
//                                     <div>
...
//                                 <Link to="/user/points-history" className="group flex items-center justify-between p-4 rounded-lg bg-white hover:bg-[#4a3a2f] hover:text-white transition-colors border border-stone-200">
//                                     <div>
...
//                                 <a href="/user/rewards" className="group flex items-center justify-between p-4 rounded-lg bg-white hover:bg-[#4a3a2f] hover:text-white transition-colors border border-stone-200">
//                                     <div>
...
//                                 <Link to="/user/privacy-policy" className="w-full flex items-center justify-between text-left p-4 rounded-lg hover:bg-stone-200 transition-colors">
//                                     <div className="flex items-center space-x-3">
...
//                                 <Link to="/user/terms-and-conditions" className="w-full flex items-center justify-between text-left p-4 rounded-lg hover:bg-stone-200 transition-colors">
//                                     <div className="flex items-center space-x-3">
...
import { LogOut, ArrowRight, User, Shield, Camera, X, Award, Users, FileText } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { getProfile, updateProfile, changePassword } from '../api/api';
import Loader from "../components/Loader";
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setGlobalError(e?.toString() || 'Failed to load profile');
      }
    };
    load();
    return () => clearTimeout(timer);
  ...
  // ------------------ Edit Profile Modal ------------------
  const EditProfileModal = ({ isVisible, closeModal }) => {
    ...
    return (
      <Modal closeModal={closeModal} isVisible={isVisible}>
        ...
        <form onSubmit={handleEditProfile} className="space-y-4">
          ...
          {editError && (
            <div className="text-red-500 text-sm text-center">{editError}</div>
          )}
          {editSuccess && (
            <div className="text-green-600 text-sm text-center">{editSuccess}</div>
          )}
          <button
            disabled={saving}
    ...
    const handleChangePassword = async (e) => {
      ...
      try {
        ...
        if (!currentPassword || !newPassword || !confirmNewPassword)
          throw new Error('Enter all password fields');
        ...
        setPwdError(e?.toString() || 'Failed to change password');
      } finally {
        setChangingPwd(false);
      }
    };
    ...
    return (
      <Modal closeModal={closeModal} isVisible={isVisible}>
        ...
        <form onSubmit={handleChangePassword} className="space-y-4">
          ...
          />
          {pwdError && (
            <div className="text-red-500 text-sm text-center">{pwdError}</div>
          )}
          {pwdSuccess && (
            <div className="text-green-600 text-sm text-center">{pwdSuccess}</div>
          )}
          <button
  ...
  return (
    <div className="min-h-screen bg-white text-[#4a3a2f] font-sans pb-24">
      <AnimatePresence>
        ...
        />
        <ChangePasswordModal
          isVisible={showChangePwd}
          closeModal={() => setShowChangePwd(false)}
        />
      </AnimatePresence>
